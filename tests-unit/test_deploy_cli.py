...
class TestParseArgs(unittest.TestCase):
    ...
    def test_short_flags(self):
        args = deploy.parse_args(["-u", "myuser", "-p", "pw", "-y", "-n"])
    ...
    def test_json_flag(self):
        args = deploy.parse_args(["-u", "user", "--json"])
        self.assertTrue(args.json)

    def test_all_flags(self):
        args = deploy.parse_args(["-u", "user", "-p", "pw", "-y", "-n", "--json"])
        self.assertTrue(args.yes)
        self.assertTrue(args.dry_run)
        self.assertTrue(args.json)
...
class TestCLIReporterJSON(unittest.TestCase):
    ...
    def test_json_mode_collects_events(self):
        cli = deploy.CLIReporter(json_mode=True)
...
class TestHandleError(unittest.TestCase):
    ...
    def test_fatal_returns_false(self):
        args = deploy.parse_args(["-u", "test", "-y"])
        cli = deploy.CLIReporter()
        with patch("sys.stdout", new_callable=StringIO):
            result = deploy.handle_error(args, cli, "Fatal error", is_fatal=True)
    ...
    def test_yes_flag_continues(self):
        ...
        cli = deploy.CLIReporter()
        with patch("sys.stdout", new_callable=StringIO):
            result = deploy.handle_error(args, cli, "Non-fatal error")
        self.assertTrue(result)

    def test_json_mode_records_error(self):
        args = deploy.parse_args(["-u", "test", "-y", "--json"])
        cli = deploy.CLIReporter(json_mode=True)
        deploy.handle_error(args, cli, "Some error")
...
class TestMainValidation(unittest.TestCase):
    ...
    def test_invalid_username_exits(self):
        with self.assertRaises(SystemExit) as ctx:
            with patch("sys.stdout", new_callable=StringIO):
                deploy.main(["--username", "INVALID_USER", "--password", "test"])
    ...
    @patch("deploy_lib.check_root", return_value=False)
    def test_non_root_exits(self, mock_root):
        with self.assertRaises(SystemExit) as ctx:
            with patch("sys.stdout", new_callable=StringIO):
                deploy.main(["--username", "testuser", "--password", "test"])
        self.assertEqual(ctx.exception.code, 1)
    ...
    @patch("deploy_lib.user_exists", return_value=False)
    @patch("deploy_lib.check_root", return_value=True)
    def test_new_user_without_password_exits(self, mock_root, mock_exists):
        with self.assertRaises(SystemExit) as _:
            with patch("sys.stdout", new_callable=StringIO):
                deploy.main(["--username", "newuser"])
...
class TestDryRun(unittest.TestCase):
    ...
    def test_dry_run_exits_zero(self, mock_exists):
        with self.assertRaises(SystemExit) as ctx:
            with patch("sys.stdout", new_callable=StringIO):
                deploy.main(["--username", "testuser", "--dry-run"])
        self.assertEqual(ctx.exception.code, 0)

    @patch("deploy_lib.user_exists", return_value=True)
    ...
    def test_dry_run_json_valid(self, mock_exists):
        with self.assertRaises(SystemExit) as ctx:
            with patch("sys.stdout", new_callable=StringIO) as mock_out:
                deploy.main(["--username", "testuser", "--dry-run", "--json"])
        output = mock_out.getvalue()
        result = json.loads(output)
