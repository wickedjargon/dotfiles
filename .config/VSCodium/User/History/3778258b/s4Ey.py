...
def get_dis():
    try:
        ...
        res = f"You are closest to {description_of_closest} which is located at {co_of_closest}"
        result.set(res)
...
Label(master, text="Enter the filename: ", bg="light grey").grid(row=1, sticky=W)
Label(master, text="Enter your location (lat): ", bg="light grey").grid(row=2, sticky=W)
Label(master, text="Enter your location (lon): ", bg="light grey").grid(row=3, sticky=W)
...
e1 = Entry(master, width=50) # filename
e1.grid(row=1, column=1)
e2 = Entry(master, width=50) # location (lat)
e2.grid(row=2, column=1)
e3 = Entry(master, width=50) # location (lon)
e3.grid(row=3, column=1)

# creating a button using the widget
b = Button(master, text="Get results", command=get_dis, bg="white")
b.grid(
    row=1,
    column=2,
    columnspan=2,
