...
while True:
    currtime = convert(time.strftime("%H:%M"))  #Gets the current time as array -> [hh, mm]
    events = [l.strip() for l in get(["calcurse", "-a"]).splitlines()][1:]  # Gets appointments for today and splits it and put to an array (try $calcurse -a)
    # arrange event data:
    groups = []
    sub = []
    for l in events:
        if l.startswith("* "):
            ...
            sub_apnd = convert(l.split("->")[0].replace("-", "").strip())
            diff = [x1 - x2 for (x1, x2) in zip(sub_apnd, currtime)]
            if diff[0] > 0 or diff[0] == 0 and diff[1] > 0:
                sub.append(sub_apnd)
        elif len(sub):
            groups.append(sub + [l])
            sub = []
    ...
    for item in groups:
        ...
        if all([len(item) == 2, not item in times]):
            ...
            if span <= warn:
                ...
                times.append(item)
    # clean up events
    startups = cleanup(startups, groups)
    times = cleanup(times, groups)
    time.sleep(30)
