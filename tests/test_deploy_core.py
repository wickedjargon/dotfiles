...
class TestDeployCore(unittest.TestCase):
    ...
    def test_deploy_root_simple(self, mock_run, mock_move, mock_copy2):
        # Test the unified deploy_root function
        import tempfile
        
        # Create a temp directory with root/ structure
        with tempfile.TemporaryDirectory() as tmpdir:
            script_dir = Path(tmpdir)
            dotfiles_dir = script_dir / "root" / "home" / "new-user"
                    ...
                    with patch('pathlib.Path.mkdir') as mock_mkdir:
                        with patch('pathlib.Path.is_symlink') as mock_is_symlink:
                            with patch('deploy.get_backup_dir') as mock_get_backup:
                                ...
                                success, error, backup, items = deploy.deploy_root(self.username, script_dir)
